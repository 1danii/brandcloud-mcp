...
export const schema = {
  ...
  fileId: z.number().describe("The ID of the file to retrieve"),
};

export const metadata: ToolMetadata = {
  name: "get-file-detail",
  description:
    "Get detailed information about a specific file including where it's used, EXIF data, previews, and history.",
  annotations: {
    title: "Get BrandCloud File Details",
    readOnlyHint: true,
...
export default async function getFileDetail({
  domain,
  fileId,
  ...
  const url = `${baseUrl}/file/${fileId}${params.toString() ? `?${params.toString()}` : ''}`;
